...
int bindInputChannel(int port, int pin)
{
    ...
    if (kernelID != -1)
    {
        ...
        if (gpiounexport(kernelID))
            return -1;
        if (gpioexport(kernelID))
            return -1;
        if (gpiosetdir(kernelID, GPIO_IN))
            return -1;
    }
    else
...
int bindOutputChannel(int port, int pin)
{
    ...
    if (kernelID != -1)
    {
        ...
        if (gpioexport(kernelID))
            return -1;
        if (gpiosetdir(kernelID, GPIO_OUT))
            return -1;
    }
    else
        fprintf(stderr, "\nError: channel J%d.%d unavailable.\n", port, pin);
...
int setChannelState(int kernelID, int state)
{
    if (state)
    {
        if (gpiosetbits(kernelID))
            return -1;
    }
    else
    {
        if (gpioclearbits(kernelID))
            return -1;
    }
    return 0;
}
